	...
	public class CameraMovement : MonoBehaviour {
        ...
        void FollowUnit () //method to follow unit
        {
            ...
            { //if the player presses the follow key
                UnitToFollow = (Unit)gameMgr.SelectionMgr.Selected.GetSingleEntity(EntityTypes.unit, true);
            }
        }
        ...
        void OnMinimapClick ()
        {
            ...
            if (Hits.Length > 0)
            {
                for (int i = 0; i < Hits.Length; i++)
                {
                    ...
                    if (gameMgr.TerrainMgr.IsTerrainTile(Hits[i].transform.gameObject))
                    {
                        //if this is the left mouse button and the selection box is disabled
                        if (Input.GetMouseButtonUp(0) && gameMgr.SelectionMgr.Box.IsActive == false)
                        {
                        ...
                        {
                            List<Unit> selectedUnits = gameMgr.SelectionMgr.Selected.GetEntities(EntityTypes.unit, true, true).Cast<Unit>().ToList(); //get selected units from player faction

                            if(selectedUnits.Count > 0)
                                //move the selected units to the new clicked position in the minimap
                                gameMgr.MvtMgr.Move(selectedUnits,Hits[i].point, 0.0f, null, InputMode.movement, true);
                        }
                    }
                }
            }
		...
		{
            Unit selectedUnit = (Unit)gameMgr.SelectionMgr.Selected.GetSingleEntity(EntityTypes.unit, true);
            if (selectedUnit)
                LookAt(selectedUnit.transform.position);
		}
