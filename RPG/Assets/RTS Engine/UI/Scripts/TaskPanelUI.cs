...
namespace RTSEngine
{
    ...
    public class TaskPanelUI
    {
        ...
        public TaskUI Add (TaskUIAttributes attributes, int categoryIndex, TaskUI.Types type = TaskUI.Types.idle)
        {
            ...
            TaskList currTaskList = (type == TaskUI.Types.multipleSelection) ? taskLists[taskLists.Length - 1]
        ...
        public void Update ()
        {
            ...
            List<Unit> selectedUnits = gameMgr.SelectionMgr.Selected.GetEntities(EntityTypes.unit, false, true).Cast<Unit>().ToList(); //get selected units from player faction
            List<Building> selectedBuildings = gameMgr.SelectionMgr.Selected.GetEntities(EntityTypes.building, false, true).Cast<Building>().ToList(); //get selected buildings from player faction

            if(selectedUnits.Count + selectedBuildings.Count == 1) //if only one faction entity is selected
            {
                FactionEntity factionEntity = selectedUnits.Count == 1 ? (FactionEntity)selectedUnits[0] : selectedBuildings[0]; //get it
            ...
            if (APCComp.IsEmpty() == false) //if there are units stored inside the APC
            {
                if (APCComp.CanEject(true) == true) //if we're allowed to eject all units at once
                    Add(new TaskUIAttributes {
                        ...
                        source = sourceEntity.GetSelection(),
        ...
        public void UpdateInProgressTasks (TaskLauncher taskLauncher)
        {
            ...
            for(int progressTaskID = 0; progressTaskID < taskLauncher.GetTaskQueueCount(); progressTaskID++)
            {
                Add(new TaskUIAttributes
                {
                    ...
                    taskLauncher = taskLauncher,
                    source = taskLauncher.FactionEntity.GetSelection(),
        ...
        public void UpdateBuilderTasks (Unit sourceUnit, Builder builderComp)
        {
            ...
            { 
                ...
                TaskUI taskUI = Add(new TaskUIAttributes
                {
                    ...
                    source = sourceUnit.GetSelection(),
