...
namespace RTSEngine
{
	public class UIManager : MonoBehaviour 
	{
        ...
        public void UpdateMultipleSelectionPanel () //update the multiple selection panel to display the selected units group
        {
            ...
            List<Entity> selectedEntities = gameMgr.SelectionMgr.Selected.GetEntities(EntityTypes.none, true, false);
            foreach (Entity entity in selectedEntities)
            {
                ...
                {
                    source = entity.GetSelection(),
                    type = TaskTypes.deselect,
                    icon = entity.GetIcon()
                    
                }, 0, TaskUI.Types.multipleSelection);
            }
        }
        ...
        private void OnEntitySelected (Entity entity)
        {
            ...
            if(gameMgr.SelectionMgr.Selected.GetCount() > 1) //more than one is selected
            {
        ...
        {
            int selectionCount = gameMgr.SelectionMgr.Selected.GetCount();
