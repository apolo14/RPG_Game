...
namespace RTSEngine
{
	...
	public class TaskUI : MonoBehaviour {
         ...
         * */
        public enum Types {idle, inProgress, multipleSelection};
        ...
        public void Reload(TaskUIAttributes attributes, Types type)
        {
            ...
            if (attributes.source && attributes.source.FactionEntity == null) //if there's a source and it is not a faction entity (i.e: resource)
                type = Types.idle; //force type to be idle
        ...
        private void Update()
        {
            ...
            switch(type)
            {
                case Types.inProgress:
                    ...
                    break;
                case Types.multipleSelection:
                    //update the progress bar to show the selected unit's health:
                    nextBarLength = attributes.source.FactionEntity.EntityHealthComp.CurrHealth / (float)attributes.source.FactionEntity.EntityHealthComp.MaxHealth;
                    break;
            }
        ...
        public void ShowTaskInfo()
        {
            ...
            {
                case TaskTypes.deselect: //deselect a currently selected unit
                    return; //do not show anything

                case TaskTypes.generateResource: //generating resource task.

                    ResourceGenerator.Generator generator = ((Building)attributes.source.FactionEntity).GeneratorComp.GetGenerator(attributes.ID);
                ...
                case TaskTypes.APCEject: //APC release task.
                    ...
                    message += "Eject unit: " + attributes.source.FactionEntity.APCComp.GetStoredUnit(attributes.ID).GetName() + " from the " + attributes.source.FactionEntity.GetName() + ".";
                    break;
                case TaskTypes.APCEjectAll: //APC release task.

                    message += "Eject all units inside the " + attributes.source.FactionEntity.GetName() + ".";
                    break;
                case TaskTypes.APCCall: //apc calling units

                    message += "Call units to get into the " + attributes.source.FactionEntity.GetName() + ".";
                    break;
        ...
        private void AddResourceInfo (ResourceManager.Resources[] resources, string title, ref string message)
        {
            if (resources.Length > 0)
            {
                message += $"\n<b>{title}:</b>";

                foreach (ResourceManager.Resources r in resources)
