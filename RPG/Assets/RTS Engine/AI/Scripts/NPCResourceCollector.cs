...
namespace RTSEngine
{
    public class NPCResourceCollector : NPCComponent
    {
        ...
        public CollectionInfo GetCollectionInfo(string resourceName)
        {
            ...
            {
                if (ci.resourceTypeInfo.GetName() == resourceName) //if the collection info matches the input name.
                    return ci;
            }
        ...
        public void UpdateResourcesToCollect(Resource resource, bool add)
        {
            if (add == true) //adding
            {
                ...
                {
                    resource = resource,
                    collectionInfo = GetCollectionInfo(resource.GetResourceType().GetName())
                };
