...
namespace RTSEngine
{
    ...
    public class NPCUnitRegulator : NPCRegulator<Unit>
    {
        ...
        public void Init(GameManager gameMgr, NPCManager npcMgr, NPCUnitCreator unitCreator_NPC)
        {
            //assign the appropriate faction manager and unit creator settings
            base.Init(gameMgr, npcMgr);
            ...
            UpdateTargetAmount(gameMgr.GetFaction(this.factionMgr.FactionID).GetMaxPopulation());
        ...
        void OnTaskLaunched(TaskLauncher taskLauncher, int taskID, int taskQueueID)
        {
            ...
            {
                if (taskLauncher.GetTask(taskID).UnitCode == code)
                {
                    //increase amount
                    amount++;
                    pendingAmount++;
