...
namespace RTSEngine
{
    public class InputManager : MonoBehaviour
    {
        ...
        private List<Entity> spawnablePrefabs = new List<Entity>(); //what prefabs can this component actually create in a multiplayer game?

        public Dictionary<int, Entity> spawnedObjects = new Dictionary<int, Entity>(); //the objects spawned by this component are saved in here.
        ...
        public static void SendInput(NetworkInput newInput, Entity source, Entity target)
        {
            if (source) //if there's a source object
            {
                if (newInput.sourceMode == (byte)InputMode.create) //if we're creating an object, then look in the spawnable prefabs list
                    newInput.sourceID = InputManager.instance.spawnablePrefabs.IndexOf(source); //get the index of the prefab from the spawnable prefabs list as the source ID
        ...
        private void OnSpawnFactionCommand(NetworkInput command)
        {
            ...
            if (gameMgr.GetFaction(command.factionID).GetTypeInfo() == null || gameMgr.GetFaction(command.factionID).GetTypeInfo().GetCapitalBuilding() == null)
            {
            ...
            //gameMgr.GetFaction(command.factionID).SpawnFactionEntities(gameMgr); //spawn the faction capital building and the default faction entity instances
        }
        ...
        private void OnCreateCommand(NetworkInput command)
        {
            ...
            switch ((InputMode)command.targetMode)
            {
                case InputMode.unit:
                    ...
                    gameMgr.UnitMgr.CreateUnitLocal(prefab as Unit, command.initialPosition, command.factionID, (creator as Building), command.value == 0, command.value == 2);
             ...
             * */
                    ...
                    gameMgr.BuildingMgr.CreatePlacedInstanceLocal(prefab as Building, command.initialPosition, command.targetPosition.y, (center as Building).BorderComp, command.factionID, placedByDefault, isCapital);
                    break;
        ...
        public static string UnitListToString(List<Unit> unitList)
        {
            ...
            foreach (Unit unit in unitList) //go through the unit list
                ...
                resultString += instance.spawnedObjects[unit.MultiplayerKey].ToString() + ",";
        ...
        public static List<Unit> StringToUnitList(string inputString)
        {
            ...
            foreach (string index in unitIndexes) //go through all the indexes
                unitList.Add(InputManager.instance.spawnedObjects[Int32.Parse(index)] as Unit); //add the unit that matches the index to the list
