...
namespace RTSEngine
{
    ...
    public class Border : MonoBehaviour
    {
        ...
        public class BuildingInBorder
        {
            ...
            public string GetFactionCode () { return factionType.GetCode(); }
            ...
            while (i < buildingsInBorder.Count) //go through the buildings in border slots
                if (buildingsInBorder[i].GetFactionCode() != "" 
                    && (gameMgr.GetFaction(building.FactionID).GetTypeInfo() == null 
                        || gameMgr.GetFaction(building.FactionID).GetTypeInfo().GetCode() != buildingsInBorder[i].GetFactionCode())) //if the faction code is specified and doesn't match the building's faction
