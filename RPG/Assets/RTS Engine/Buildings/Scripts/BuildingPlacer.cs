...
namespace RTSEngine
{
    ...
    public class BuildingPlacer : MonoBehaviour
    {
        ...
        public void PlaceBuilding()
        {
            ...
            gameMgr.MinimapIconMgr.AssignIcon(building.GetSelection()); //ask the minimap icon manager to create the a minimap icon for this building
            ...
            List<Unit> selectedUnits = gameMgr.SelectionMgr.Selected.GetEntities(EntityTypes.unit, true, true).Cast<Unit>().ToList();

            if (building.FactionID == GameManager.PlayerFactionID && GodMode.Enabled == false && selectedUnits.Count > 0 && building.PlacedByDefault == false) //if the player owns this building and this is a single player game
            {
        ...
        public bool IsBuildingNearResource()
        {
            ...
            foreach (Resource resource in gameMgr.ResourceMgr.GetAllResources()) //go through all resources
                if (resource == resourceType) //make sure this is the resource that the building needs (matching names)
