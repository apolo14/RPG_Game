    ...
    public class UnitAttack : AttackEntity
    {
            ...
            if (moveOnAttack == true)
                return Vector3.Distance(transform.position, GetTargetPosition()) 
                    <= gameMgr.MvtMgr.GetStoppingDistance() 
                        + RangeType.GetStoppingDistance(Target == null ? EntityTypes.none : Target.Type) + ((moveOnAttack == true) ? RangeType.GetMoveOnAttackOffset() : 0);
            else
        ...
        protected override void OnTargetUpdate()
        {
            if (Target?.Type == EntityTypes.unit) //if there's a target unit
            {
                ...
                if (Vector3.Distance(lastTargetPosition, GetTargetPosition()) >= RangeType.GetUpdateMvtDistance())
                {
