...
namespace RTSEngine
{
    public abstract class AttackEntity : MonoBehaviour
    {
        ...
        public ErrorMessage CanEngageTarget (FactionEntity potentialTarget)
        {
            ...
            else if (gameMgr.InPeaceTime() == true)
        ...
        protected virtual void FixedUpdate ()
        {
            ...
            //if the attack component is not active, is locked or the attack entity can't attack or the game is still in peace time
            if (isActive == false || isLocked || CanEngage() == false || gameMgr.InPeaceTime() == true) 
                return;
        ...
        private void SearchTarget ()
        {
            ...
            foreach (Unit enemyUnit in (FactionEntity.IsFree() == true || engageFriendly) ? gameMgr.UnitMgr.GetAllUnits() : FactionEntity.FactionMgr.GetEnemyUnits())
            {
                if (enemyUnit == null || enemyUnit.enabled == false || enemyUnit.HealthComp.IsDead() == true || enemyUnit == FactionEntity as Unit)
                ...
                if((currDistance = Vector3.Distance(enemyUnit.transform.position, searchCenter)) < searchSize && CanEngageTarget(enemyUnit) == ErrorMessage.none)
                {
