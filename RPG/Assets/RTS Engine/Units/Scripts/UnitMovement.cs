...
namespace RTSEngine
{
    ...
    public class UnitMovement : MonoBehaviour
    {
        ...
        public void Init(GameManager gameMgr, Unit unit)
        {
            ...
            navAgent.updatePosition = false; //since we'll be manually controlling the unit's position, we don't need the nav agent component to update the position
        ...
        public void OnPathComplete(Vector3 targetPosition, Entity target, Vector3 lookAtTarget, float stoppingDistance, InputMode targetMode)
        {
            ...
            lookAtTransform = (target != null) ? target.transform : null;
            ...
            if (target && target.gameObject.activeInHierarchy == true) //if the unit is moving towards an active object
            {
                ...
                FactionEntity targetFactionEntity = target as FactionEntity;
                Resource targetResource = target as Resource;

                if (targetFactionEntity)
                {
                    if (unit.AttackComp && unit.AttackComp.CanEngageTarget(targetFactionEntity) == ErrorMessage.none) //if the unit is set to attack the target object
