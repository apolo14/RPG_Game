...
namespace RTSEngine
{
	public class APC : MonoBehaviour {
		...
		public bool Add (Unit unit)
		{
            ...
            gameMgr.SelectionMgr.Selected.Remove(unit.GetSelection()); //deselect unit in case it was selected
		...
		public void CallUnits ()
		{
			...
			foreach (Unit u in FactionEntity.FactionMgr.GetUnits()) { //go through the faction's units
                ...
                if (storedUnits.Count >= capacity) //if there are no longer free positions
                    break; //stop
