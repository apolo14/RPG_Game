...
namespace RTSEngine
{
	public class UnitManager : MonoBehaviour {
        ...
        {
            if (source.IsFree()) //if the source unit was free
                freeUnits.Remove(source); //remove it from the free units list
        }

        public Unit CreateUnit(Unit unitPrefab, Vector3 spawnPosition, int factionID, Building createdBy, bool free = false, bool updatePopluation = false)
        {
            if (GameManager.MultiplayerGame == false) //single player game 
                return CreateUnitLocal(unitPrefab, spawnPosition, factionID, createdBy, free, updatePopluation); //directly create new unit instance
        ...
        public Unit CreateUnitLocal (Unit unitPrefab, Vector3 spawnPosition, int factionID, Building createdBy, bool free = false, bool updatePopulation = false)
        {
            ...
            newUnit.Init(gameMgr, factionID, free);

            if(!free && updatePopulation) //if this is not a free unit and we need to update the population slots
            {
