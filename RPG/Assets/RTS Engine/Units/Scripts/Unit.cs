...
namespace RTSEngine
{
    ...
    public class Unit : FactionEntity
    {
        ...
        public override void Init(GameManager gameMgr, int fID, bool free)
        {
            ...
            if (AttackComp)
                AttackComp.Init(gameMgr, this, MultipleAttackMgr);
            ...
            if (!this.free)
            {
                if ((RTSHelper.IsLocalPlayer(this) || GameManager.MultiplayerGame == false) && Creator != null)
            ...
            gameMgr.MinimapIconMgr.AssignIcon(selection); //ask the minimap icon manager to create the a minimap icon for this unit
        ...
        public void ConvertLocal(Unit converter, int targetFactionID)
        {
            ...
            if (selection.IsSelected)
                gameMgr.SelectionMgr.Selected.Remove(selection);
        ...
        public void SetAnimState(UnitAnimState newState)
        {
            ...
            if (currAnimatorState == UnitAnimState.takingDamage && HealthComp.IsDamageAnimationEnabled() == false) //if taking damage animation is disabled
                return;
