...
namespace RTSEngine
{
    public class MovementManager : MonoBehaviour
    {
        ...
        public struct MovementTask
        {
            ...
            public bool attack;
            public FactionEntity attackTarget;
        ...
        public List<Vector3> GenerateTargetPositions(Formations formation, Vector3 originPosition, int amount, float unitRadius, LayerMask agentAreaMask, bool isFlyUnit, ref float offsetRadius, Vector3 unitsDirection)
        {
            if (isFlyUnit == true) //if this is a flying unit
                originPosition.y = gameMgr.TerrainMgr.GetFlyingHeight(); //set the position to the flying height.
                ...
                switch (formation)
                {
                    case Formations.circular: //in case we have a circular formation
                        ...
                        while (subCounter < expectedPositionCount) //as long as we haven't inspected all the expected free positions inside this cirlce
                        {
                            ...
                            Vector3 possiblePosition = new Vector3(x, gameMgr.TerrainMgr.SampleHeight(new Vector3(x, originPosition.y, z), unitRadius, agentAreaMask), z);
                    ...
                    case Formations.rectangular:
                        ...
                        {
                            pos = new Vector3(pos.x, gameMgr.TerrainMgr.SampleHeight(pos, unitRadius, agentAreaMask), pos.z);
        ...
        private void OnMovementRequest(MovementTask movementTask)
        {
            ...
            else //if a valid path is found
            {
                ...
                if (movementTask.attack == true) //attack movement
                    movementTask.unit.AttackComp.SetTargetLocal(movementTask.attackTarget, movementTask.attackTargetPosition);
        ...
        public void LaunchAttackLocal(Unit unit, FactionEntity targetEntity, Vector3 targetPosition, AttackModes attackMode, bool playerCommand)
        {
            PrepareMove(unit, targetPosition, targetEntity ? targetEntity.GetRadius() : 0.0f, targetEntity,
                InputMode.attack, playerCommand, true, attackMode, targetEntity);
        }
        ...
        public bool CanProceedAttackMovement (Unit unit, bool playerCommand, bool assignedAttackOnly, bool canChangeTarget, FactionEntity targetEntity)
        {
            ...
            ErrorMessage nextError = ErrorMessage.attackTargetNoChange;
            ...
            //... or the unit can't engage with this particular target if it exists
            if ((assignedAttackOnly == true && unit.AttackComp.CanEngageOnAssign() == false)
                || (unit.AttackComp.Target != null && canChangeTarget == false) 
                || (nextError = unit.AttackComp.CanEngageTarget(targetEntity)) != ErrorMessage.none)
            {
                if (playerCommand == true && unit.FactionID == GameManager.PlayerFactionID) //player notification message
                    ErrorMessageHandler.OnErrorMessage(nextError, unit);
                return false;
            }
        ...
        public Vector3 GetRandomMovablePosition(Vector3 origin, float range, Unit unit, LayerMask areaMask)
        {
            ...
            randomDirection.y = gameMgr.TerrainMgr.SampleHeight(randomDirection, range, areaMask);
