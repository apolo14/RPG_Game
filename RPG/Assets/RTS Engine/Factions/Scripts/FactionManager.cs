...
namespace RTSEngine 
{
	public class FactionManager : MonoBehaviour {
        ...
        public List<FactionLimit> limits = new List<FactionLimit>();
		...
		public void Init (GameManager gameMgr, int ID, IEnumerable<FactionLimit> factionLimits) {
		...
		private void RemoveUnit (Unit unit)
		{
            ...
            if (unit.AttackComp)
            {
                ...
                UpdateCurrentAttackPower(unit, false);
            }
            else
        ...
        //check whether all required buildings to spawn a building are built
        public bool CheckRequiredBuildings(Building building)
        {
            //the building requirement array is an array of arrays, one building must be built and spawned from each array element in order to meet the requirements
            //so we'll be going through each array element, one by one

            foreach (Building.RequiredBuilding list in building.GetRequiredBuildings())
            {
                List<string> buildingCodes = list.GetBuildingList().Select(b => b.GetCode()).ToList<string>(); //get the required buildings codes

                if (buildingCodes.Count == 0) //no code specified? move to the next requirement
                    continue;

                bool found = false;

                foreach(Building b in buildings) //go through the faction's spawned buildings
                {
                    if (b.IsBuilt && buildingCodes.Contains(b.GetCode())) //if this building instance is built and it has one of the required codes
                    {
                        found = true;
                        break;
                    }
