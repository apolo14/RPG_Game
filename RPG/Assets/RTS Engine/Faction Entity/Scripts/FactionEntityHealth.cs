    ...
    public abstract class FactionEntityHealth : MonoBehaviour
    {
        ...
        public virtual void OnHealthUpdated (int value, FactionEntity source)
        {
            if (value < 0.0)
            {
                ...
                if (factionEntity.FactionID == GameManager.PlayerFactionID && gameMgr.AttackWarningMgr != null)
                    gameMgr.AttackWarningMgr.AddAttackWarning(this.gameObject); //show attack warning on minimap
            }
        ...
        {
            gameMgr.SelectionMgr.Selected.Remove(factionEntity.GetSelection()); //deselect the faction entity if it was selected
