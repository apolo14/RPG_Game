...
namespace RTSEngine
{
    public class UnitSelection : SelectionEntity
    {
        ...
        {
            List<Building> selectedBuildings = gameMgr.SelectionMgr.Selected.GetEntities(EntityTypes.building, false, true).Cast<Building>().ToList(); //get a list of the selected buildings
            List<Unit> selectedUnits = gameMgr.SelectionMgr.Selected.GetEntities(EntityTypes.unit, false, true).Cast<Unit>().ToList(); //get the list of selected units
            ...
            if (selectedUnits.Count > 0) //units selected
            {
                foreach (Unit selectedUnit in selectedUnits)
                {
                    ...
                    if (selectedUnit.ConverterComp && (taskType == TaskTypes.none || taskType == TaskTypes.convert))
                    {
                        ...
                        {
                            flashSelection = true;
                            audioClip = unit.ConverterComp.GetOrderAudio();
                            continue;
                        }
                    }
