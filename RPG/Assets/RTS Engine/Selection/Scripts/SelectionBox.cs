...
namespace RTSEngine
{
    ...
    public class SelectionBox
    {
        ...
        {
            if (Vector3.Distance(finalMousePosition, initialMousePosition) < minSize) //if the box's size is too small
                return; //do not continue

            //deselect currently selected entities
            manager.Selected.RemoveAll();

            /*if (manager.MultipleSelectionKeyDown == false) //if the player is not holding down the multiple selection key down
                manager.DeselectUnits(); //deselect units*/
            ...
            {
                Vector3 unitScreenPosition = Camera.main.WorldToScreenPoint(unit.GetSelection().transform.position); //get the unit's position on screen
                ...
                {
                    manager.Selected.Add(unit.GetSelection(), SelectionTypes.multiple); 
                }
            }
        }
